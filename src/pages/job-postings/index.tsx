...
import { useRoqClient, useJobPostingFindManyWithCount } from 'lib/roq';
...
export function JobPostingListPage(props: JobPostingListPageProps) {
  ...
  const queryParams = useMemo(
    () =>
      convertQueryToPrismaUtil(
        {
          relations: ['company', 'applicant.count'],
  ...
  const { data, error, isLoading, mutate } = useJobPostingFindManyWithCount({
  ...
  const handleDelete = async (id: string) => {
    setDeleteError(null);
    try {
      await roqClient.job_posting.delete({ where: { id } });
      await mutate();
    } catch (error) {
